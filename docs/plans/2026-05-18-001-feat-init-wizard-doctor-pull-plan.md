...
- **Init and doctor share a `src/init/detection.rs` module** with `detect_hardware()` (calls `gpu::probe` + RAM/disk inspection), `detect_binary()` (wraps `launch::binary::locate` + R54 common-location probes), and `verify_binary_integrity()` (sha256 + resolve-symlink-then-validate the real target: owner must be us or root, +x, parent-dir-mode checks). doctor consumes the same module read-only.
...
- **`--yes` is `--no-confirm` semantics, not "answer Y to everything".** A failing integrity check (parent-dir writable, foreign-UID-owned target, archive bomb, checksum mismatch) aborts with exit 72/73/74 rather than silently downgrading. Reasoning: `--yes` is the agent's path; agents must see deterministic failure.
...
- Create: `src/init/install/custom_path.rs` — accept an explicit `llama-server` path; run the same integrity checks (resolve a symlink to its real target, require that target owned by us or root, +x bit, parent-dir writability, SHA-256 record). A root-owned `/usr/bin` symlink — the system-package-manager install — is adopted; warn when the basename isn't a server binary (e.g. `llama-cli`).
...
- "Point at existing binary" path: the install module also owns the integrity check (resolve symlink → target owner us-or-root, parent-dir + target mode, digest record). `--yes` mode auto-pre-selects an existing binary only if integrity passes; failure aborts with exit 72.
...
- *Integration*: custom-path with parent dir mode 0777 is refused; a symlink to a foreign-UID-owned target is refused; a symlink to a same-user-or-root target in a safe dir is accepted and the resolved target's digest recorded.
