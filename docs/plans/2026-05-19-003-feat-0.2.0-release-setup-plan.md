...
- **Install script's served copy is a content-verified mirror, not a redirect.** GitHub Pages can't serve HTTP 30x redirects that `curl` honors, so `llamastash.dev/install.sh` is regenerated by the site repo's bump workflow by fetching the GH Release asset and SHA-256-verifying before commit. Trust boundary is the bump workflow's verification step; branch protection on `.github/workflows/bump.yml` plus required reviews on workflow changes is the operational mitigation. (See origin: R91, Key Decision #4.)
...
- Create: `astro.config.mjs` — Tailwind integration, `site: 'https://llamastash.dev'`, no SSR (`output: 'static'`).
...
- Create: `public/CNAME` — exactly `llamastash.dev`, one line, no trailing newline (GitHub Pages quirk).
...
- Create: `README.md` — short, points at `llamastash.dev` and the main repo.
...
- Happy path: pushing to `main` triggers `deploy.yml`; the deploy succeeds within the 30s target; the deployed site renders at `llamastash.github.io` (and at `llamastash.dev` after Unit 7 lands DNS).
...
**Test scenarios:**
- Happy path: visiting `llamastash.github.io` (or `llamastash.dev` after Unit 7) renders all eight sections in Macchiato palette.
...
- [ ] **Unit 7: CNAME flip + first real tag + smoke verification** *(pre-tag file changes prepared: Cargo.toml at `0.2.0`, CHANGELOG `[Unreleased]` → `[0.2.0]` promotion, README install commands now reference live channels + llamastash.dev. Tag-push, cli.rs DNS PR, and fresh-box smoke testing remain pending Unit 2 + org-admin bootstrap)*

**Goal:** Land `llamastash.dev → llamastash.github.io` DNS via PR to the community-managed `cli.rs` zone-config repo, flip the site to serve under the branded URL, cut the actual `v0.2.0` tag, and smoke-test every install path on a fresh box. This is the public-launch unit.
...
- Modify: `llamastash/llamastash.github.io` repo's `public/CNAME` — already committed in Unit 5; this unit's role is to confirm it's still `llamastash.dev` and that the file is in `dist/` after build.
- Modify: `llamastash/README.md` — replace any "binaries not yet published" disclaimer with the three live install commands; mention `llamastash.dev` as the marketing URL.
...
**Approach:**
- Open the cli.rs PR first; while waiting for it to merge, the site is already live at `llamastash.github.io` (Units 5+6). Marketing copy that mentions `llamastash.dev` is held until DNS resolves.
- Once cli.rs DNS resolves: verify with `dig llamastash.dev` returning a CNAME pointing at `llamastash.github.io`. GitHub Pages auto-provisions an HTTPS cert via Let's Encrypt within a few minutes of CNAME working.
...
**Test scenarios:**
- Happy path: `dig llamastash.dev` returns a CNAME to `llamastash.github.io`; `curl https://llamastash.dev` returns 200 with valid TLS.
...
- Happy path on fresh Ubuntu: `curl -fsSL https://llamastash.dev/install.sh | sh` succeeds; binary on `~/.local/bin/llamastash`.
- Happy path on fresh macOS: `brew install llamastash/llamastash/llamastash` succeeds; binary runs.
- Happy path on fresh macOS: `curl -fsSL https://llamastash.dev/install.sh | sh` succeeds; no Gatekeeper prompt (since curl path is quarantine-free per the origin's macOS analysis).
- Edge case: cli.rs DNS takes hours/days to propagate after PR merge. Marketing copy that mentions `llamastash.dev` is held; release proceeds with `llamastash.github.io` as the temporarily-canonical URL; copy is amended once DNS is live.
...
- `https://llamastash.dev` returns 200 with valid TLS and renders the Catppuccin Macchiato site.
...
- **GitHub Pages cert provisioning:** After the CNAME for `llamastash.dev` resolves to `llamastash.github.io`, GitHub Pages auto-provisions a Let's Encrypt certificate. Documented turnaround is "up to 24 hours" but most cases land within minutes. **During the provisioning window**, users hitting `https://llamastash.dev` get a TLS error. Mitigation: Unit 7 verifies cert validity before announcing the URL in README and CHANGELOG. If the provisioning hangs > 24h, the workaround is to toggle the "Enforce HTTPS" setting in the site repo's Pages settings (off then on) — documented in the [GitHub Pages docs on custom domains](https://docs.github.com/en/pages/configuring-a-custom-domain-for-your-github-pages-site).
...
| CNAME for `cli.rs` takes longer than expected to provision; community-zone PR sits in review queue for days. | Medium | Low | Site is live at `llamastash.github.io` from Unit 5 onward; marketing copy and crate description that mention `llamastash.dev` are held until DNS resolves; release is not gated on cli.rs DNS. README install commands at tag time use whichever URL is live (either `llamastash.dev` or fallback to the GH Releases URL directly — both work for install.sh). A late DNS flip is purely additive; nothing breaks. |
