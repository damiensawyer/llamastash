...
_Source: 12 run file(s) from host(s) deepu-flowz13-arch, deepu-flowz13-arch-clean70w, deepu-flowz13-arch-hip-rocwmma-off, deepu-flowz13-arch-hip-rocwmma-on, deepu-flowz13-arch-large-dense-vulkan, deepu-flowz13-arch-lms-vulkan, deepu-flowz13-arch-rocm, deepu-flowz13-arch-vulkan on GPU backend(s) rocm; engine variants in this page: HIP/ROCm, HIP+rocWMMA, Vulkan._
...
> | ---------------- | --------------------------- | --------- |
> | `-rocm` (= the plain `deepu-flowz13-arch` row also) | local **HIP** build of llama.cpp **9245 (`b39a7bf1b`)** (`build-hip/bin/llama-server`) — see *"HIP build flags caveat"* below | bundled `amd-rocm-avx2` **v2.16.0** |
> | `-vulkan` | local **Vulkan** build of the same llama.cpp **9245 (`b39a7bf1b`)** (`build-vulkan/bin/llama-server`) | bundled `vulkan-avx2` **v2.16.0** |
>
...
> | ------------------------------- | -------------------------------: | --------: |
> | HIP / ROCm (9245 vs v2.16.0)    | ~81 tok/s                        | ~91 / ~80 (chat / agent) |
> | Vulkan (9245 vs v2.16.0)        | ~95 tok/s                        | ~91 / ~80 (chat / agent) |
>
...
>    on either backend.
>
> 2. **For our local 9245 builds, Vulkan is ~17–20% faster than HIP
>    at decode.** [measured]
>
...
>
>    | Build (9245) | chat_turn | agent_decode |
>    | ------------- | --------: | -----------: |
...
>    essentially the same vintage as our local 9245 builds. The actual
...
| llama-server (raw) | defaults | `deepu-flowz13-arch-large-dense-vulkan` | 7.5 tok/s | 770.4 ms | 62.3 tok/s | 3 | ok |
...
| llama-server (raw) | normalized | `deepu-flowz13-arch-large-dense-vulkan` | 7.5 tok/s | 758.5 ms | 63.3 tok/s | 3 | ok |
...
| LlamaStash | defaults | `deepu-flowz13-arch-large-dense-vulkan` | 7.4 tok/s | 785.6 ms | 61.1 tok/s | 3 | ok |
...
| LlamaStash | normalized | `deepu-flowz13-arch-large-dense-vulkan` | 7.5 tok/s | 761.4 ms | 63.1 tok/s | 3 | ok |
...
| llama-server (raw) | defaults | `deepu-flowz13-arch-large-dense-vulkan` | 7.5 tok/s | 719.5 ms | 58.4 tok/s | 3 | ok |
...
| llama-server (raw) | normalized | `deepu-flowz13-arch-large-dense-vulkan` | 7.5 tok/s | 721.4 ms | 58.3 tok/s | 3 | ok |
...
| LlamaStash | defaults | `deepu-flowz13-arch-large-dense-vulkan` | 7.4 tok/s | 743.3 ms | 56.6 tok/s | 3 | ok |
...
| LlamaStash | normalized | `deepu-flowz13-arch-large-dense-vulkan` | 7.5 tok/s | 701.2 ms | 60.0 tok/s | 3 | ok |
| LM Studio | defaults | `deepu-flowz13-arch` | — | — | — | 0 | ok |
...
| llama-server (raw) | defaults | `deepu-flowz13-arch-large-dense-vulkan` | 25.2 tok/s | 2,189.5 ms | — | 3 | ok |
...
| llama-server (raw) | normalized | `deepu-flowz13-arch-large-dense-vulkan` | 25.3 tok/s | 2,204.3 ms | — | 3 | ok |
...
| LlamaStash | defaults | `deepu-flowz13-arch-large-dense-vulkan` | 25.2 tok/s | 2,226.1 ms | — | 3 | ok |
...
| LlamaStash | normalized | `deepu-flowz13-arch-large-dense-vulkan` | 25.2 tok/s | 2,224.2 ms | — | 3 | ok |
| LM Studio | defaults | `deepu-flowz13-arch` | — | — | — | 0 | ok |
...
| llama-server (raw) | defaults | `deepu-flowz13-arch-large-dense-vulkan` | 7.3 tok/s | 197.7 ms | 41,730.5 tok/s | 3 | ok |
...
| llama-server (raw) | normalized | `deepu-flowz13-arch-large-dense-vulkan` | 7.3 tok/s | 204.0 ms | 40,380.2 tok/s | 3 | ok |
...
| LlamaStash | defaults | `deepu-flowz13-arch-large-dense-vulkan` | 7.3 tok/s | 203.6 ms | 40,456.3 tok/s | 3 | ok |
...
| LlamaStash | normalized | `deepu-flowz13-arch-large-dense-vulkan` | 7.3 tok/s | 203.7 ms | 40,447.1 tok/s | 3 | ok |
| LM Studio | defaults | `deepu-flowz13-arch` | — | — | — | 0 | ok |
