...
pub struct HostMetricsSnapshot {
  ...
  /// Per-device rows. Each entry carries a backend-prefixed selector
  /// (`Nvidia0`, `Amd0`, `Vulkan0`, `Metal0`) that maps to the
  /// `--device` flag value, plus the device name and total VRAM for
  /// display in the device picker. On Apple Silicon and CpuOnly this
  /// carries a single synthetic row.
  #[serde(default, skip_serializing_if = "Option::is_none")]
...
fn build_snapshot(sys: &System, components: &Components, info: GpuInfo) -> HostMetricsSnapshot {
  ...
  let (picker, stats) = build_device_rows(&info);
  HostMetricsSnapshot {
    cpu_pct,
    cpu_temp_c,
...
/// Build per-device rows from the probe result. Each row carries a
/// backend-prefixed selector (`Nvidia0`, `Amd0`, `Vulkan0`,
/// `Metal0`) that maps directly to the `--device` flag, plus the
/// device name, VRAM (used/total), utilization, and temperature
/// when the vendor tool surfaces them. Also returned as the
/// `gpu_devices` field so the TUI host stats pane can render
/// per-GPU rows.
pub fn build_device_rows(info: &GpuInfo) -> (Option<Vec<DeviceRow>>, Option<Vec<DeviceRow>>) {
  let all: &[GpuDevice] = match info {
    GpuInfo::CpuOnly => return (None, None),
    GpuInfo::AppleMetal { total_memory_bytes } => {
  ...
  // For the host stats pane, return the same rows (they now carry
  // all the data needed for per-GPU display).
  (Some(picker.clone()), Some(picker))
}
