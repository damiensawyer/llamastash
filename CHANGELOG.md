...
- TUI: key hints render non-bold across the top bar, the confirm popup, and the help overlay; the top bar gains an always-on `↑↓:scroll` chip; and the right pane's tab strip dims to muted + first-letter-underlined (non-bold) when the pane is unfocused, matching the Models pane title.
