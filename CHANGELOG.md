...
- OpenAI-compat proxy router on `127.0.0.1:11434` (enabled by default) — single URL, model-name routing through the existing fuzzy resolver, auto-start of dormant models, single-flight launch coalescing, family-MRU fallback on launch failure with `x-llamastash-served-by` + `x-llamastash-fallback-reason: launch_failed` headers. Loopback only, no auth, no TLS. Surfaces through `status.proxy` (IPC + CLI `--json`) and the TUI Daemon info pane. See [`docs/usage.md §Proxy (OpenAI-compatible listener)`](docs/usage.md#proxy-openai-compatible-listener) and [`docs/plans/2026-05-21-001-feat-proxy-router-plan.md`](docs/plans/2026-05-21-001-feat-proxy-router-plan.md).
