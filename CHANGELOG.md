...
- **Lemonade backend (experimental, opt-in)** — drive [Lemonade](https://github.com/lemonade-sdk/lemonade) (`lemond`) as a second backend for NPU / multi-engine inference. LlamaStash finds a user-installed `lemond` (`lemonade.binary` or `PATH`), supervises it as a shared umbrella, lists its registry models in the catalog tagged `lemonade`, delegates start/stop through its load/unload API (forwarding `ctx` + extras as load options), and routes OpenAI-compat traffic to it through the proxy; idle eviction unloads the model while the umbrella stays up, and daemon restarts ride out a dying predecessor instead of refusing the port. Off by default — enable via `lemonade.enabled`, `--lemonade`, or `LLAMASTASH_LEMONADE=1`; setup in [docs/lemonade-setup.md](docs/lemonade-setup.md). (#28)
