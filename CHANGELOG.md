...
- TUI with grouped list + favorites + filter, launch picker, typed-knob launch-flag editor, clipboard yank, streaming Chat / Embed / Rerank / Logs right pane, five themes (Catppuccin Macchiato default).
- CLI: `list` / `start` / `stop` / `status` / `logs` / `presets` / `favorites` / `daemon` — every read+mutation command supports `--json`, with documented exit codes and an auto-spawn-daemon flow (`--no-spawn` to opt out). `start <model> -- <flags>` and `presets save -- <flags>` parse typed-knob flags (`--threads`, `-ngl`, `--cache-type-k`, …) into `knobs`; unknown flags route to `extras`.
- `llamastash init` first-run wizard (R48): detect → install `llama-server` per OS×GPU → recommend + pull a GGUF → smoke launch → TUI handoff. Per-step `--install` / `--model` / `--config-step` overrides, `--recommended` / `--json` / `--offline` modes, and `--revision <SHA>` to pin HF commits. No longer writes `arch_defaults` to `config.yaml` — the built-in defaults table supersedes it; YAML stays as a hand-edited escape hatch.
...
- Layered launch-flag resolver with source labels — `preset > last_params > yaml arch_defaults > built-in table > llama-server`. Each resolved field carries a `LayerLabel` so the Settings tab can render per-row origin chips (`(user)`, `(last used)`, `(arch default)`, `(model default)`, `(server default)`).
